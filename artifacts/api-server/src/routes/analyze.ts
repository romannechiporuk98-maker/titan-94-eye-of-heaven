...
router.post("/analyze", async (req, res) => {
  ...
  const knowledge = await store.listKnowledge(5);
...
${knowledge.map((k) => `- ${k.category}: ${k.pattern}`).join("\n")}

Return JSON only:
{"severity":"critical|high|medium|low|none","score":0-100,"vulnerabilities":[{"type":"...","severity":"...","description":"...","impact":"..."}],"summary":"2 sentences","recommendation":"1 sentence fix","honeypot_risk":"low|medium|high","model":"gemini-2.0-flash"}`;
    ...
    } else result = keywordScan(contractCode);
  } else {
  ...
  await store.logActivity(
    "ANALYZE",
    `Contract analyzed: ${(address || "unknown").slice(0, 16)}...`,
    `Severity: ${result.severity} | Score: ${result.score}/100 | ${result.vulnerabilities?.length || 0} issues`,
    result.severity === "none" ? "info" : result.severity,
    { address, model: result.model },
  );
...
Return JSON only:
{"is_honeypot":true|false,"confidence":0.0-1.0,"risk_level":"safe|low|medium|high|critical","red_flags":["flag1"],"verdict":"1 sentence","safe_to_interact":true|false}`;
    ...
    if (raw) {
      try {
        ...
        result = { is_honeypot: false, confidence: 0.5, risk_level: "medium", red_flags: ["Analysis parse failed"], verdict: "Could not determine — manual review needed", safe_to_interact: false };
      }
    } else {
  ...
  await store.logActivity("ANALYZE", `HoneyPot check: ${address.slice(0, 16)}...`, result.verdict, result.is_honeypot ? "high" : "info");
